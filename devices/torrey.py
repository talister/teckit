# -*- coding: utf-8 -*-
# $Id: devices/torrey.py | Rev 44  | 2020/02/13 19:28:25 tin_fpga $
...
class tec_meter():
    ...
    def __init__(self,gpib,reflevel,name):
        self.gpib = gpib
        print("\033[4;5H \033[0;32mUART[\033[1m%2d\033[0;32m] : Arroyo TECpak\033[0;39m" % self.gpib)
        #if cfg.get('teckit', 'interface', 1) == 'gpib':
        ...
        self.inst = serial.Serial(port='/dev/ttyUSB0', baudrate = 2400, parity=serial.PARITY_NONE, timeout=0.1)
    ...
    def cfg_temp(self):
        self.inst.write("A 30.0\r") #set temp
        time.sleep(0.8)
        tec_rtd = 30.0
    ...
    def set_tmp(self,tmp):
        ...
        self.inst.write("A %2.1f\r" % string) 
        time.sleep(15)
	...
	#self.inst.clear()
        time.sleep(1)
        self.status_flag,data = self.read_data("a\r")
        time.sleep(1)
        self.status_flag,data = self.read_data("a\r")
        if (self.status_flag):
